...
class Controller:
    ...
    def check_for_new_genes(self):
        if self.flagMessage == True:
            ...
            # Reset light flag
            # self.lightOn = False
            # print("if light is on: {}".format(controller.lightOn))
    ...
    def calculate_fitness(self):
        ...
        lineTrackFitness = 100 * (temp_line - 0.25)
        ...
        # when detect an obstacle on the right-hand side, turn left significantly
        elif (0.0622 < self.inputs[3] < 0.65 or 0.0622 < self.inputs[4] < 0.65) and self.velocity_right - self.velocity_left >= 1.5:
            avoidCollisionFitness = 1.3
        # when detect an obstacle on the left-hand side, turn right significantly
        elif (0.0622 < self.inputs[7] < 0.65 or 0.0622 < self.inputs[8] < 0.65) and self.velocity_left - self.velocity_right >= 1.5:
            avoidCollisionFitness = 1.3
        # when parallel with the obstacle, moving forward with slight curve
        elif (0.2 < self.inputs[5] < 0.65 or 0.2 < self.inputs[6] < 0.65) and abs(self.velocity_right - self.velocity_left) < 1.5:
            avoidCollisionFitness = 1.3
        # no obstacle case
        elif max(self.inputs[3:9]) < 0.0667:
            avoidCollisionFitness = 1
        else:
            avoidCollisionFitness = 0.5

        ########### Forward (0~1)
        forwardFitness = (self.velocity_left + self.velocity_right) * 6     # 0.6
        # print("velocity: left {}, right{}".format(self.velocity_left, self.velocity_right))
        # print("forwardFitness value:  " + str(forwardFitness))

        ########### Spinning (0~1)
        spinningFitness = abs(self.velocity_left - self.velocity_right) / 2 * 10    # 2
        ...
        # 2. Special Case: On line, hitting obstacle (explicit)
        # if avoidCollisionFitness < 0.1 and

        ###########
        ### DEFINE the fitness function equation of this iteration which should be a combination of the previous functions
        ### 1. Line from "*" to "+"
        # combinedFitness = lineTrackFitness + forwardFitness*(1 - math.sqrt(spinningFitness)) + (1 - avoidCollisionFitness -0.2)
        combinedFitness = (lineTrackFitness + forwardFitness + (1 - math.sqrt(spinningFitness))) * avoidCollisionFitness

        # print(lineTrackFitness + forwardFitness + (1 - math.sqrt(spinningFitness)) * avoidCollisionFitness)
    ...
    def handle_receiver(self):
        if self.receiver.getQueueLength() > 0:
            while (self.receiver.getQueueLength() > 0):
                ...
                self.receivedCross = self.receivedData[-2]
                self.receivedNextTrial = self.receivedData[-1]
                self.receivedData = self.receivedData[11:-3]
                # print(self.receivedNextTrial)
                # print(self.receivedData)
